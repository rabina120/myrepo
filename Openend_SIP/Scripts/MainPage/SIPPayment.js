...
function ShHolder(data) {
    var self = this;
    if (data != undefined) {/// <reference path="../../Login.Master" />
...
var SipPaymentViewModel = function () {
   ...
   self.GTotal= ko.observable();
    ...
    var ProductIdentity;
     ...
     self.GatewayCode=ko.observable('003');
      //Payment
    self.TXNID = ko.observable();

    var PayMarchantId = $("#ContentPlaceHolder1_PayMarchantId").val();
    var PayAppIdSIPPayment = $("#ContentPlaceHolder1_PayAppIdSIPPayment").val();
    self.PayMarchantId=ko.observable(PayMarchantId);         
    self.PayAppId=ko.observable(PayAppIdSIPPayment);      

    self.PayAppName=ko.observable('SIP Payment');
    self.PayTxnDate=ko.observable(today);
    ...
    var SIPPaymentURL = $("#ContentPlaceHolder1_SIPPaymentURL").val();
    self.TestURL = ko.observable(SIPPaymentURL);  // To Pass URL Link of mutual fund
    ...
    self.LoadDp();
     ...
     self.CheckSHolders = function () {
        ...
        if (self.BOIDValidation()) {
            ...
            ProductIdentity = Bo_Account_No.toString();
                  ...
                  var data = jQuery.parseJSON(result);
                   if (data.ResponseData.length > 0) {
                     var SHolder = data.ResponseData[0];
                      ...
                      var name = ""
                      for(var i=0 ; i < SHolder.Name.length ; i++){
                            if(i%2 == 1 ){
                                name += '*'
                            }else{
                                name+=SHolder.Name[i]
                      ...
                      var contact1 = ""
                      for(var i=0 ; i < SHolder.Contact1.length ; i++){
                            if(i%2 == 1 ){
                                contact1 += '*'
                            }else{
                                contact1+=SHolder.Contact1[i]
                            }
                      }
                      var contact2 = ""
                      for(var i=0 ; i < SHolder.Contact2.length ; i++){
                            if(i%2 == 1 ){
                                contact2 += '*'
                            }else{
                                contact2+=SHolder.Contact2[i]
                            }
                      }
...
//     self.ValidateInterval =function(){
//          for (var i = 0; i < self.ShHolders().length; i++) {
//            var x = i + 1;
//            var Check =  $($('#tblfund').DataTable().row(i).nodes()).find('input').prop('checked');
//            if (Check != undefined && Check != "" && Check != false) {
...
//          }
//          var url = "../../Handler/SIPHandling/SIPPayment.ashx";
//          var data = { 'method': 'ValidateInterval', 'args': JSON.stringify(self.UserList())  };
//          $.post(url, data,
//                        function (result) {
 ...
 function VerifyKhaltiPayment(payload) {
  ...
  self.UpdateToken = function(){
      ...
      //To open Khalti payment form with Config
        var config = {
            "publicKey": Khaltiref,
            "productName": "SIP Payment",
            "productUrl": SIPPaymentURL,
             "paymentPreference": [
                "KHALTI"
                ],
            "eventHandler": {
                   ...
                   //--scd --
                    ...
                    form.appendChild(pid);
                     ...
                     var su =document.createElement('input');
                         ...
                         su.setAttribute('value', SIPPaymentURL+'?q=su');
                    ...
                    form.appendChild(su);
                     ...
                     var fu =document.createElement('input');
                         ...
                         fu.setAttribute('value',SIPPaymentURL+'?q=fu');
...
//                                });
  ...
  if( getUrlParamVal('q') =="su"){
    ...
    self.Clearcontrol = function () {
        ...
        self.InstallDate('');
        self.Sip_amt(null);
        self.Interval_Seq_No(null);
        self.ShHolderNo(null);
        self.SIP_Reg_No(null);
        self.Name(null);
        self.Contact1(null);
        self.Contact2(null);
        self.TotalAmount(null);
